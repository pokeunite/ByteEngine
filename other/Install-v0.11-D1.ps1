...
function Fail([string]$Message) {
    throw "ByteEngine v0.11-D1 installer: $Message"
}
...
function Assert-CleanTrackedFile(
    ...
    if ($actual -ne $ExpectedBlob) {
        Fail "$RelativePath does not match the v0.11-D1 baseline. Expected $ExpectedBlob, got $actual."
    }
}
...
$fieldsNew = @'
    ...
    private const float BrowserTileWidth =
        122.0f;

    private const float BrowserTileHeight =
        108.0f;
...
Write-Host ""
Write-Host "v0.11-D1 first-pass facelift installed." -ForegroundColor Green
Write-Host ""
