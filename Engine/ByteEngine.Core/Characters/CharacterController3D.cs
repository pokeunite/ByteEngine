...
public sealed class CharacterController3D : Component
{
    ...
    public float AirControl { get; set; } = .35f;
    public float JumpForce { get; set; } = 7f;
    public float Gravity { get; set; } = 20f;
    public float GroundDistance { get; set; } = .15f;
    public float MaxSlope { get; set; } = 50f;
    public float StepHeight { get; set; } = .3f;
    public float CoyoteTime { get; set; } = .1f;
    public float JumpBuffer { get; set; } = .1f;
    ...
    protected override void OnUpdate()
    {
        ...
        Vector3 target = _moveInput;
        target.Y = 0f;
        if (target.LengthSquared() > 1f) target = Vector3.Normalize(target);
        target *= MoveSpeed;

        float control = IsGrounded ? 1f : AirControl;
        float rate = target.LengthSquared() > .001f ? Acceleration : Deceleration;
        Velocity = new Vector3(
            Approach(Velocity.X, target.X, rate * control * deltaTime),
            Velocity.Y,
            Approach(Velocity.Z, target.Z, rate * control * deltaTime));
        ...
        if (!IsGrounded) Velocity += Vector3.UnitY * (-Gravity * deltaTime);
    ...
    private void DetectGround()
    {
        ...
        {
            foreach (GameObject item in GameObject.Scene.GameObjects)
            {
                GroundSurface? ground = item.GetComponent<GroundSurface>();
                if (ground?.Walkable != true) continue;
                float height = item.Transform.WorldPosition.Y;
                if (height <= Transform.WorldPosition.Y + GroundDistance && height > bestHeight)
                {
                    bestHeight = height;
                    GroundObject = item;
                }
        ...
        IsGrounded =
            GroundObject != null &&
            Transform.WorldPosition.Y <= bestHeight + GroundDistance &&
            Velocity.Y <= 0f;
        if (IsGrounded && SnapToGround)
        {
            Transform.WorldPosition = new Vector3(
