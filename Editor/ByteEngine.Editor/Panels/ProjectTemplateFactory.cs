...
internal static class ProjectTemplateFactory
{
    ...
    private static Scene CreateByteArena()
    {
        ...
        ground.AddComponent(new BoxCollider3D { Size = new Vector3(1f, .1f, 1f), Center = new Vector3(0f, -.05f, 0f) });
        ...
        player.AddComponent(new CharacterController3D { MoveSpeed = 5f, JumpForce = 7f });
        player.AddComponent(new HealthComponent { MaxHealth = 100f, CurrentHealth = 100f });
        player.AddComponent(new ProjectileLauncher3D
        {
            ProjectileSpeed = 25f,
            Damage = 25f,
            FireCooldown = .25f,
            MuzzleOffset = new Vector3(0f, 0f, -.8f)
        });
        player.AddComponent(new PlayerController3D());
        player.AddComponent(new PlayerShooter3D());

        GameObject camera = scene.CreateGameObject("Main Camera");
        camera.Transform.LocalPosition = new Vector3(0f, 5f, 12f);
        camera.AddComponent(new Camera3D { FieldOfView = 65f });
        ...
        for (int index = 0; index < enemyPositions.Length; index++)
        {
            ...
            {
                Primitive = PrimitiveMeshType.Cube,
                Material = new Material { BaseColor = new Vector4(.9f, .18f, .12f, 1f) }
            });
            enemy.AddComponent(new BoxCollider3D { Size = Vector3.One });
            enemy.AddComponent(new HealthComponent { MaxHealth = 50f, CurrentHealth = 50f });
        ...
        light.AddComponent(new DirectionalLight { Intensity = 1.25f, AmbientIntensity = .4f });

        GameObject manager = scene.CreateGameObject("Game Manager");
        manager.AddComponent(new ArenaGameManager { PlayerId = player.Id, PlayerName = player.Name });
        return scene;
    }
}
